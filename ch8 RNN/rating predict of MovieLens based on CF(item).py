...
def compute_pearson_similarity(ratings_matrix, based="item"):
    """
    计算皮尔逊相关系数
...
def predict(uid, iid, ratings_matrix, item_similarity):
    ...
    # 首先找出电影的相似电影(去掉自身和nan)
    similar_items = item_similarity[iid].drop([iid]).dropna()
    # 相似物品的筛选：只留下相似度大于0（正相关）的物品
    similar_items = similar_items.where(similar_items > 0).dropna()
    if similar_items.empty is True:
        raise Exception(f"电影{int(iid)}没有相似的电影")

    # 从相似电影中找出具有对iid进行评分记录的用户uid评分过的其它电影
    ids = set(ratings_matrix.loc[uid].dropna().index) & set(similar_items.index)
    finally_similar_item = similar_items.loc[np.array(list(ids), dtype=np.int64).tolist()]

    # 结合uid用户和其相似用户预测对iid的评分
    # 评分计算的分子和分母
    sum_up = 0
    sum_down = 0
    for sim_iid, similarity in finally_similar_item.items():
        # 相似物品的评分数据
        sim_item_rated_movies = ratings_matrix[sim_iid].dropna()
        # 用户对相似物品的评分
        sum_item_rating_for_user = sim_item_rated_movies[uid]
        # 计算公式中分子的值
        sum_up += similarity * sum_item_rating_for_user
        # 计算公式中分母的值
        sum_down += similarity
